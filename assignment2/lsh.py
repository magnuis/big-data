...
def lsh(m_matrix):
    ...
    for band_index in range(len(m_matrix_banded[0])):   
        ...
        for signature_index, signature in enumerate(m_matrix_banded):
            ...
            hash_value = bucket_hash(a=a, b=buckets, c=c, x=signature[band_index])
...
def count_false_neg_and_pos(lsh_similarity_matrix, naive_similarity_matrix):
    ...
    for similarity in naive_similarity_matrix:
        if similarity >= t:
            actual_positive += 1

    false_negatives = actual_positive - (positive - false_positives)
